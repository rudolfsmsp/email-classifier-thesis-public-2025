...
def store_user_provided_email(email_text, label):
    """
    Store user-provided email (text + final label) in user_provided_emails.csv
    """
        ...
        try:
            ...
            if os.path.exists(file_path):
                with open(file_path, "r", encoding="utf-8") as f:
                    reader = csv.reader(f)
                    existing_entries = {tuple(row) for row in reader if len(row) == 3}
...
def main():
    ...
    st.markdown(
        ...
        **Classify Email**, the system uses trained machine learning models to predict the category.
        ...
        If you disagree with the prediction, you can override it. If you consent, your email text and final label will be stored 
        in a CSV file (**user_provided_emails.csv**) to improve the model.

        <br><br>
        <span class='gold-text'>Safe email</span> – a genuine message from a trusted sender that doesn't try to trick you.<br>
        <span class='gold-text'>Spam email</span> – an unwanted message with ads or promotions, but not seeking personal data.<br>
        <span class='gold-text'>Phishing email</span> – a scam that pretends to be from a reputable company or bank to steal sensitive info.<br><br>
        <strong>How to differentiate spam and phishing:</strong><br>
        - Spam is primarily bulk advertising, usually not seeking personal data.<br>
        - Phishing uses urgent requests, fake links, or asks for sensitive information to trick you.<br><br>
    ...
    if not st.session_state.predicted:
        if st.button("Classify Email"):
            ...
            with st.spinner("Classification in-progress..."):
                ...
                # If a phishing URL is found, override with "Phishing Email"
                if risk_value == 2:
                    st.warning("Warning: Unsafe URL detected, automatically flagging the input as Phishing.")
    ...
    if st.session_state.predicted:
        ...
        if agree == "No":
            ...
            if consent:
                ...
                time.sleep(2)
                st.success("User-provided email stored successfully.")
